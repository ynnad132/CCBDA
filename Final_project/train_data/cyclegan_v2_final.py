...
G_AB = build_res_unet(n_input=1, n_output=2, size=128)    # l -> ab
D_B = Discriminator(input_c=2, n_down=3, num_filters=64)

G_BA = build_res_unet(n_input=2, n_output=1, size=128)    # ab -> l
D_A = Discriminator(input_c=1, n_down=3, num_filters=64)
...
def sample_images(A_L, A_ab, B_L, B_ab, figside=1.5, epoch=0):
    ...
    fake_A_ab = G_AB(A_L).detach()

    B_L = B_L.type(Tensor).to(device)
    B_ab = B_ab.type(Tensor).to(device)
    fake_B_L = G_BA(B_ab).detach()

    fake_A = lab_to_rgb(A_L, fake_A_ab)
    real_A = lab_to_rgb(A_L, A_ab)
    fake_B = lab_to_rgb(fake_B_L, B_ab)
...
sample_images(A_L, A_ab, B_L, B_ab)

# In[]

from tqdm import tqdm

save_path_AB = os.getcwd() + '/model/CycleGAN/G_AB_epoch_'
save_path_BA = os.getcwd() + '/model/CycleGAN/G_BA_epoch_'
...
for epoch in range(n_epoches):
    for i, (A_L, A_ab, B_L, B_ab) in enumerate(tqdm(trainloader)):
        ...
        loss_real = criterion_GAN(D_A(A_L), True)
        loss_fake = criterion_GAN(D_A(fake_B_L.detach()), False)
    ...
    if (epoch+1) % 5 == 0:
        ...
        sample_images(test_A_L, test_A_ab, test_B_L, test_B_ab, epoch=epoch+1)
        ...
        torch.save(G_AB, save_path_AB+str(epoch+1)+'.pt')
        torch.save(G_BA,save_path_BA+str(epoch+1)+'.pt')
...
save_path = os.getcwd() + 'save_img'
#model_save_path = os.getcwd() + '/model/G_' + style + '.pt'
model_save_path = r'C:\Users\EE803\Desktop\train_data-20230108T154630Z-001\train_data\model\CycleGAN\G_AB_epoch_20'
transforms = torchvision.transforms.Resize((size, size))

#net_G = model_def.build_res_unet(n_input=1, n_output=2, size=128)
G_AB = torch.load_state_dict(model_save_path, map_location=device)
G_AB.eval()
...
for idx in range(43):
    ...
    img_lab = torchvision.transforms.ToTensor()(img_lab).to(device)
    ...
    save_name = save_path + '/' + img_name + '_' + style + '.jpg'
