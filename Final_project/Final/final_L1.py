# -*- coding: utf-8 -*-
"""
Created on Sat Jan  7 15:16:44 2023
...
train_paths = paths[:7980] # choosing the first 8000 as training set
val_paths = paths[7980:] # choosing last 2000 as validation set
...
def train_model(model, train_dl, epochs, display_every=200):
    ...
    save_path = os.getcwd() + '/model/NetG.pt' 
    torch.save(model.net_G, save_path)
...
net_G = torch.load(model_save_path)
net_G.eval()
...
for idx in range(43):
    ...
    img = np.array(img)
    img_lab = rgb2lab(img).astype("float32") # Converting RGB to L*a*b
    img_lab = transforms.ToTensor()(img_lab).to(device)
    real_L = img_lab[[0], ...] / 50. - 1. # Between -1 and 1
    real_L = real_L.unsqueeze(0)
    with torch.no_grad():
        fake_ab = net_G(real_L)
    fake_img = lab_to_rgb(real_L, fake_ab).squeeze(0)*255
